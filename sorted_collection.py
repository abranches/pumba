...
class SortedCollection(object):
    ...
    def find_lt(self, k, *args, **kwargs):
        'Return last item with a key < k.  Raise ValueError if not found.'
        i = bisect_left(self._keys, k, *args, **kwargs)
        if i:
            return i, self._items[i-1]
    ...
    def slice_items(self, min, max):#, fail_on_not_found_min=False):
        try:
            left_i, _ = self.find_ge(min)
        except ValueError:
            #if fail_on_not_found_min:
            #    raise
            left_i = len(self._items)

        try:
            right_i, _ = self.find_lt(max, lo=left_i)
        except ValueError:
            right_i = left_i#len(self._items)
            # this will return []

        return self._items[left_i:right_i]
